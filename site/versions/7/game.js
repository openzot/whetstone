 ...
 *
 * Pass 7 (world): six passes in, the run had no journey in it. The light
 * never changed - a 3000 m drive looked exactly like a 300 m one, noon
 * forever, so the shelf's one road still had no places in it. Now the world
 * runs on a clock driven by distance (a pure function of px travelled, like
 * the centreline): a run leaves midday behind into amber dusk, spends a long
 * stretch in true night where the tarmac goes blue-black and you read the
 * road by your own headlight cone, the bloom of tail-lights, the lamp pools
 * ahead of traffic and the reflective glint of the delineator posts - then
 * climbs back through rose dawn into morning if it lasts long enough. The
 * world dims but never blacks out: nothing about collisions, spawning,
 * steering or sound moved, only where you are.
 *
 ...
 * and the wreck (5), weighted steering with body roll (6).
 *
...
(function () {
  ...
  function drawCar(u, s, w, h, body, isPlayer, nk) {
    ...
    // pool warm on the tarmac just ahead, and traffic throws a faint beam of
    // its own up the road so oncoming reads before the body does.
    if (nk > 0.03) {
  ...
  function draw() {
    ...
    if (nk > 0.02) {
      ...
      ctx.fill();
      ctx.globalCompositeOperation = "lighter";
      ctx.globalAlpha = nk * 0.22;
      ctx.fillStyle = "#ffedbe";
      ctx.beginPath();
      ctx.ellipse(nx, ny - 26, 30, 34, 0, 0, Math.PI * 2);
