 ...
 *
 * Pass 29 (world): twenty-eight passes gave this highway bends (3), a day
 * clock (7), other drivers (9), storms (11), tunnels and underpasses (16),
 * viaducts over open water (24), redline zones (20) and a neon downtown
 * (26) - but not one line of it ever says where you are or what is ahead.
 * Driving version 28 I crossed the great water bridge and ran into the lit
 * towers of the first district with zero warning of either: the only text
 * on the roadside was numbered half-kilometre posts, so the world's places
 * arrive unannounced and every stretch between them is anonymous tarmac.
 * The road now speaks. Overhead gantries span the lanes with a lit green
 * destination board counting down to the next district; verge boards name
 * each tunnel, underpass and viaduct before its mouth stands ahead; an
 * amber hazard board warns of a redline zone; and between the stations
 * small route boards give the open road seeded names, so no kilometre is
 * anonymous. Every sign is a pure function of world distance - placed off
 * the same CITY_/TUN_/BR_/ZONE_ schedules as the places themselves, its
 * countdown live against totalS - so boards stand at the same stations
 * every run and tick down as you close on them; when a lead would land on
 * another structure's ground it walks back to the nearest free shoulder
 * instead of vanishing, two boards never crowd one stretch, and the faces
 * are painted after the night and rain veils with a lamp glow once darkness
 * holds - the zone gates' bargain - so signage reads at midnight by
 * headlight. The gantry beam passes OVER you as you run under it. Purely
 * drawn: physics, spawning, collisions, scoring, audio, state() and the
 * title screen are untouched.
 *
...
(function () {
   ...
   * Twenty-six passes built a scoring grammar - shave close for up to 100
   * pts, chain it x5, double it in a redline zone - but the grammar had no
   * word for the signature act of every dodge game: splitting two cars. A
   * pass is judged only on ITS OWN clearance, so threading the needle paid
   * precisely what two unrelated shaves paid, and the bravest line in the
   * vocabulary earned nothing a lazy diagonal did not. Playing version 26,
   * seven sighted lives totalling ~4 km scored points equal to metres to the
   * digit - chains, zones and shaves never once fired for honest mid-lane
   * driving - and even when they did, a simultaneous split was scored no
   * differently from two passes three seconds apart. Now the split is its
   * own event: two close passes on OPPOSITE flanks inside THREAD_WINDOW of
   * each other are a thread - the second pays THREAD_BONUS per live chain
   * step on top of its shave, and the pop says so ("+116 thread", stacking
   * with a zone's doubling). Same-side pairs chain as ever but never thread;
   * a lone shave pays exactly what it always paid. Nothing about physics,
   * traffic, spawning, collisions or sound moved - only what a pass is
   * worth, and one new reason to aim for the gap instead of around it. */
  var THREAD_WINDOW = 0.9;    // s between opposite-flank nose-crossings
  var THREAD_BONUS = 80;      // flat prize per chain step for the split
  ...
  var lastPassSide = 0;     // flank of the last close pass: -1 left, +1 right
  var lastPassT = -9;       // when it happened (run clock) - a thread needs both
  var threads = 0;          // splits landed this run (read-only via state())
  ...
  function init() {
    ...
    bestStreak = 0;
    chainWarned = false;
    lastPassSide = 0;   // no pass survives the grid, so no thread can either
    lastPassT = -9;
    threads = 0;
    ...
    for (var i = traffic.length - 1; i >= 0; i--) {
      ...
      if (c.prevRel !== undefined && c.prevRel > 0 && rel <= 0) {
        ...
        if (clear < CLOSE_MAX) {
          ...
          // the split: this pass and the last one rode OPPOSITE flanks inside
          // THREAD_WINDOW - you went through the gap between them, not around
          // one machine - so the thread pays its own prize per live chain step.
          // Same-side pairs keep chaining as ever; a lone shave pays exactly
          // what it always paid.
          var side = c.u >= player.u ? 1 : -1;   // which flank this car passed on
          var threaded = lastPassSide !== 0 && side !== lastPassSide &&
            worldT - lastPassT <= THREAD_WINDOW;
          lastPassSide = side;
          lastPassT = worldT;
          var pts = closeBonus(Math.max(0, clear)) * mult;
          if (threaded) { pts += THREAD_BONUS * mult; threads += 1; }
  ...
  /* --- signage: the road learns to say where it goes ------------------------

   * Twenty-eight passes built places - tunnels, viaducts over open water, a
   * neon downtown, rich ground - but the road never once pointed at one.
   * Driving version 28, I crossed the great water bridge and ran into the
   * first lit towers with no warning of either: the only text on the verge
   * was numbered half-kilometre posts, so the world's places arrive
   * unannounced and every stretch between them is anonymous tarmac. Now the
   * highway speaks. An overhead GANTRY spans the lanes before each downtown
   * with a green destination board whose metres tick down live; verge boards
   * name every tunnel, underpass and viaduct by a seeded name before its
   * mouth stands ahead; an amber hazard board warns of the next redline
   * zone; and small route boards give the open stretches seeded names so no
   * kilometre is anonymous. Each sign's station is a pure function of world
   * distance (the place's own cycle start minus a lead), so the same boards
   * stand in the same spots every run - and when a lead would land on
   * another structure's ground it walks back in 40 px steps to the nearest
   * free shoulder rather than vanish, because a tunnel must not swallow the
   * announcement of the bridge behind it. A spacing rule keeps two boards
   * off one stretch. Signs paint after the night and rain veils - the
   * bargain the zone gates struck - so their faces stay lit and readable at
   * midnight, gaining a lamp glow when darkness holds; the gantry beam is
   * overhead ironwork, passing OVER you as you run beneath it. Nothing
   * listens to any of it: physics, spawning, collisions, scoring, audio,
   * state() and the title screen are untouched. */
  var SIGN_LEAD_CITY = 1150;   // gantry stands this far before downtown
  var SIGN_LEAD_LONG = 760;    // verge board before a tunnel or viaduct
  var SIGN_LEAD_ZONE = 560;    // hazard board before a redline zone
  var SIGN_SLOT = 2600;        // px between route-name boards (~390 m)
  var SIGN_GAP = 430;          // two verge boards never share one stretch

  var CITY_NAMES = ["DOWNTOWN", "RIVERSIDE", "OLD HARBOUR", "NORTHGATE"];
  var TUN_NAMES = ["RIVERSIDE", "QUARRY HILL", "HARBOURVIEW", "OLD MILL",
    "STATION"];
  var BRIDGE_NAMES = ["KESTREL", "COAST", "BROOK", "SALT", "MILL"];
  var AREA_NAMES = ["WESTHARBOUR", "LONGMEADOW", "REDSTONE", "KINGS CROSS",
    "LOW FIELDS", "EASTGATE", "THE HEATH", "MARSH POINT"];

  // free shoulder for a board: off every structure's ground (a tunnel is
  // roofed, a crossing has water for verges). Downtown counts as ground -
  // city streets carry signage like anywhere else.
  function signFree(s) {
    return tunnelAt(s).k <= 0.02 && bridgeAt(s).k <= 0.02;
  }

  // walk a station back to free ground, up to 600 px; null if none
  function signStand(s) {
    for (var t = s; t > s - 600; t -= 40) {
      if (signFree(t)) return t;
    }
    return null;
  }

  function signCandidates(minS, maxS) {
    var out = [], c, i;
    var lo = minS - 90, hi = maxS + 90;

    for (c = Math.floor((minS - CITY_SKED) / CITY_LEN) - 1;
         c * CITY_LEN <= maxS + CITY_SKED + CITY_LEN; c++) {
      var cs = c * CITY_LEN + CITY_SPAN[0] - CITY_SKED;
      var gs = signStand(cs - SIGN_LEAD_CITY);
      if (gs === null || gs < lo || gs > hi) continue;
      out.push({
        s: gs, evt: cs, kind: "gantry",
        label: CITY_NAMES[Math.floor(hash(c * 7 + 3) * CITY_NAMES.length)]
      });
    }

    for (c = Math.floor((minS - TUN_SKED) / TUN_LEN) - 1;
         c * TUN_LEN <= maxS + TUN_SKED + TUN_LEN; c++) {
      for (i = 0; i < TUN_SPANS.length; i++) {
        var ts = c * TUN_LEN + TUN_SPANS[i][0] - TUN_SKED;
        var tb = signStand(ts - SIGN_LEAD_LONG);
        if (tb === null || tb < lo || tb > hi) continue;
        out.push({
          s: tb, evt: ts, kind: "board", face: "green",
          label: TUN_NAMES[Math.floor(hash(c * 3 + i * 11 + 5) *
            TUN_NAMES.length)] +
            (TUN_SPANS[i][1] - TUN_SPANS[i][0] > 700 ? " TUNNEL" : " UNDERPASS")
        });
      }
    }

    for (c = Math.floor((minS - BR_SKED) / BR_LEN) - 1;
         c * BR_LEN <= maxS + BR_SKED + BR_LEN; c++) {
      var bs = c * BR_LEN + BR_SPAN[0] - BR_SKED;
      var bb = signStand(bs - SIGN_LEAD_LONG);
      if (bb === null || bb < lo || bb > hi) continue;
      out.push({
        s: bb, evt: bs, kind: "board", face: "green",
        label: BRIDGE_NAMES[Math.floor(hash(c * 13 + 9) *
          BRIDGE_NAMES.length)] + " VIADUCT"
      });
    }

    var z0 = Math.floor((minS - SIGN_LEAD_ZONE - 600) / ZONE_LEN) - 1;
    var z1 = Math.floor(maxS / ZONE_LEN) + 1;
    for (c = z0; c <= z1; c++) {
      var zs = c * ZONE_LEN + 500 + hash(c * 13 + 5) * 1700;
      var zb = signStand(zs - SIGN_LEAD_ZONE);
      if (zb === null || zb < lo || zb > hi) continue;
      out.push({ s: zb, evt: zs, kind: "board", face: "amber",
        label: "REDLINE ZONE" });
    }

    // the open road gets a name too: seeded route boards on a fixed cadence
    var k0 = Math.floor((minS - SIGN_SLOT) / SIGN_SLOT);
    var k1 = Math.floor((maxS + SIGN_SLOT) / SIGN_SLOT);
    for (c = k0; c <= k1; c++) {
      var ms = c * SIGN_SLOT + hash(c * 17 + 2) * 900;
      if (hash(c * 23 + 8) < 0.35) continue;           // gaps, like trees
      if (!signFree(ms)) continue;
      if (ms < lo || ms > hi) continue;
      out.push({
        s: ms, evt: ms, kind: "board", face: "green",
        label: "REDLINE A1 \u00b7 " +
          AREA_NAMES[Math.floor(hash(c * 29 + 4) * AREA_NAMES.length)],
        area: true
      });
    }

    // spacing: two verge boards never crowd one stretch. Sorted by station,
    // a board is dropped within SIGN_GAP of a kept non-gantry sign; a
    // gantry always stands (it hangs centred overhead, not beside anything).
    out.sort(function (a, b) { return a.s - b.s; });
    var kept = [];
    for (i = 0; i < out.length; i++) {
      var cand = out[i], near = false;
      if (!near) kept.push(cand);
    }
    return kept;
  }

  // One green/amber face with its label row and, when it announces an
  // event ahead, a live countdown row ticking against totalS.
  function drawSignFace(x, y, w, lines, face, dk) {
    var h = lines.length > 1 ? 24 : 14;
    ctx.fillStyle = face === "amber" ? "#6a4a06" : "#175138";
    ctx.fillRect(x, y - h, w, h);
    ctx.strokeStyle = face === "amber" ? "#ffd75e" : "#dfeee4";
    ctx.lineWidth = 1;
    ctx.strokeRect(x + 0.5, y - h + 0.5, w - 1, h - 1);
    ctx.fillStyle = face === "amber" ? "#ffefb8" : "#f4faf5";
    ctx.font = "bold 9px monospace";
    ctx.textAlign = "left";
    ctx.fillText(lines[0], x + 4, y - (lines.length > 1 ? 12 : 10));
    if (lines.length > 1) {
      ctx.fillStyle = face === "amber" ? "#e8cf8e" : "#bfe3cd";
      ctx.font = "8px monospace";
      ctx.fillText(lines[1], x + 4, y - 3);
    }
    // lit signage earns its keep after dark: a soft halo once night holds
    if (dk > 0.04) {
      ctx.globalCompositeOperation = "lighter";
      ctx.globalAlpha = dk * 0.16;
      ctx.fillStyle = face === "amber" ? "#ffd75e" : "#7fd6a4";
      ctx.beginPath();
      ctx.ellipse(x + w / 2, y - h / 2, w * 0.62, h * 1.15, 0, 0, Math.PI * 2);
      ctx.fill();
      ctx.globalCompositeOperation = "source-over";
      ctx.globalAlpha = 1;
    }
  ...
  function drawSigns(minS, maxS, nk, tk) {
    var signs = signCandidates(minS, maxS);
    if (!signs.length) return;
    var dk = Math.max(nk, tk * 0.96);       // the veil factor machines use
    var i;

    for (i = 0; i < signs.length; i++) {
      var sg = signs[i];
      var y = PLAYER_Y - (sg.s - totalS);   // screen line of the post base
      if (y < -80 || y > H + 110) continue;
      var xm = W / 2 + roadCenter(sg.s);
      var mLeft = Math.max(0, Math.round((sg.evt - totalS) * 0.15));
      var sub = sg.area ? null : mLeft + " m";

      if (sg.kind === "gantry") {
        // legs outside both painted edges, beam across the lanes, board
        // hanging centred with the countdown in its own right-hand cell
        var legL = xm - HALF - 30, legR = xm + HALF + 30;
        var beamY = y - 54;
        ctx.fillStyle = mixHex("#454c55", "#23272e", dk);
        ctx.fillRect(legL - 2, beamY, 4, y - beamY);
        ctx.fillRect(legR - 2, beamY, 4, y - beamY);
        ctx.fillRect(legL - 2, beamY, legR - legL + 4, 5);
        ctx.fillStyle = mixHex("#8b939e", "#3a414b", dk);
        ctx.fillRect(legL - 2, beamY, legR - legL + 4, 1.5);

        ctx.font = "bold 10px monospace";
        var tw = ctx.measureText(sg.label).width;
        var cellW = 34;
        var bw = tw + cellW + 16;
        var bx = xm - bw / 2;
        drawSignFace(bx, beamY + 27, bw, [sg.label], "green", dk);
        ctx.fillStyle = "#dfeee4";
        ctx.fillRect(bx + tw + 6, beamY + 8, 1, 17);
        ctx.fillStyle = "#ffe9a8";
        ctx.font = "bold 9px monospace";
        ctx.textAlign = "right";
        ctx.fillText(mLeft + "m", bx + bw - 5, beamY + 21);
        ctx.textAlign = "left";
      } else {
        // verge board on the post style the half-kilometre markers set:
        // right shoulder, one dark post, face turned at the driver
        var vx = xm + HALF + 34;
        ctx.fillStyle = mixHex("#3c424b", "#191d23", dk);
        ctx.fillRect(vx + 8, y - 40, 3, 40);
        ctx.font = "bold 9px monospace";
        var vw = Math.max(ctx.measureText(sg.label).width + 8,
          sub ? 52 : 0);
        drawSignFace(vx, y - 38, vw, sub ? [sg.label, sub] : [sg.label],
          sg.face, dk);
      }
    }
    ctx.globalAlpha = 1;
  ...
  function draw() {
    ...
    // --- signage: the road, saying where it goes -----------------------------
    // Painted after the veils (the zone gates' bargain) so the boards stay
    // lit and readable at midnight, but before the rain so weather still
    // falls over them; the gantry's beam passes over you as you run under.
    drawSigns(minS, maxS, nk, tk);
  ...
  window.game = {
    ...
    state: function () {
      return {
        ...
        th: threads,                                     // splits landed this run
