 ...
 *
 * Pass 6 (feel): five passes in, steering was still a switch. Measured on
 * version 5 with a per-frame trace of the car's centre, the first frame
 * after keydown already carried the full 300 px/s of lateral velocity and
 * the frame after keyup carried none - lane changes began and ended as
 * instant steps, like typing, not driving; and since pass 4 pays for
 * threading gaps by pixels of clearance, the one input that sets those
 * pixels was the crudest thing in the game. Now the car has weight: your
 * input sets a target sideways velocity that the car builds up to (about
 * 80ms at launch pace) and settles down from (~150ms after you release, so
 * the car glides to rest instead of stopping dead), and both grow heavier
 * as the ramp climbs - near top speed committing takes about 130ms to come
 * on, so fast lane changes must be begun earlier and read as commitments.
 * The body rolls into the turn up to ~6 degrees with the current sideways
 * velocity, so weight transfer is visible while you steer. The old instant
 * rate is unchanged as the ceiling: same 300 px/s maximum, same clamp at
 * the painted edge, same centrifugal shove in bends.
 *
